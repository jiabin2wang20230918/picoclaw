...
func Run(opts Options) (*Result, error) {
	...
	picoClawHome, err := resolveQuantClawHome(opts.QuantClawHome)
	if err != nil {
		return nil, err
	}
	...
	fmt.Println("Migrating from OpenClaw to QuantClaw")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Description: "convert OpenClaw config to QuantClaw format",
			})
...
func executeConfigMigration(srcConfigPath, dstConfigPath, picoClawHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing QuantClaw config: %w", err)
		}
...
func resolveQuantClawHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("QUANTCLAW_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".quantclaw"), nil
}
