...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in QuantClaw.
...
10. [Creating a New Provider in QuantClaw](#creating-a-new-provider-in-quantclaw)
...
- Go ≥ 1.21
- QuantClaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.quantclaw/auth.json`:
...
## Creating a New Provider in QuantClaw

QuantClaw providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/quantclaw/cmd_auth.go`:
...
quantclaw auth login --provider your-provider

# List models (for Antigravity)
quantclaw auth models

# Start the gateway
quantclaw gateway

# Run an agent with a specific model
quantclaw agent -m "Hello" --model your-model
```
...
export QUANTCLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export QUANTCLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `quantclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.quantclaw/auth.json`
- Re-run `quantclaw auth login --provider antigravity`
