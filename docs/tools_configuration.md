...
QuantClaw's tools configuration is located in the `tools` field of `config.json`.
...
By default, QuantClaw blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `QUANTCLAW_TOOLS_<SECTION>_<KEY>`:

For example:
- `QUANTCLAW_TOOLS_WEB_BRAVE_ENABLED=true`
- `QUANTCLAW_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `QUANTCLAW_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
